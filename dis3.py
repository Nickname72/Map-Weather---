...
OPENWEATHERMAP_API_KEY = "1a61ee3445e9c64367cd8d49289388a1" # Замініть на свій ключ OpenWeatherMap, якщо цей перестане працювати
DEFAULT_LOCATION = (50.4501, 30.5234) # Київ, Україна
DEFAULT_ZOOM = 6
APP_USER_AGENT = "py_map_weather_app_v1.2_academic"
...
os.makedirs(BACKGROUNDS_DIR, exist_ok=True) # Створення папки backgrounds, якщо її немає
...
class SearchWorker(QThread):
    """Потік для виконання імітованого (заглушка) пошуку AI-асистента."""
    ...
    def run(self):
        try:
            time.sleep(1.0) # Імітація затримки
            
            log_message(f"AI: Виконання запиту: {self.query}")
            search_result = self._perform_simulated_country_search(self.query)
            
            if not search_result:
                self.result_ready.emit("Вибачте, я не зміг знайти інформацію про цей запит. Спробуйте інше формулювання.")
                return

            response_text = f"**🌐 Інформація за запитом: '{self.query}'**\n\n{search_result}"
            self.result_ready.emit(response_text)
            
        except Exception as e:
            log_message(f"AI ERROR: Помилка під час пошуку: {e}")
            self.error_occurred.emit("Сталася помилка під час виконання запиту.")

    def _perform_simulated_country_search(self, query: str) -> str:
        """
        Розширена заглушка, яка імітує відповідь AI про країни та міста.
        """
        lower_query = query.lower().strip()
        
        # --- База знань: Міста та Історія (для демонстрації) ---
        KNOWN_CITIES = {
            "вінниця": ("**Розташування:** Центральна Україна, на берегах Південного Бугу.\n"
                        "**Історія:** Перша згадка датується **1363 роком**. Назва, ймовірно, походить від слова 'віно' (посаг) або 'винокурня'. "
                        "Вінниця була важливим прикордонним містом у складі Великого князівства Литовського, а потім Речі Посполитої. "
                        "**Сучасність:** Відома своїм Європейським фонтаном Roshen, який є одним із найбільших у Європі."),
            "kyiv": ("**Location:** Capital of Ukraine, situated on the Dnieper River.\n"
                     "**History:** One of the oldest cities in Eastern Europe, founded in the **5th century**. It was the historical center of Kyivan Rus', "
                     "a medieval East Slavic state. Key historical sites include the Kyiv Pechersk Lavra and St. Sophia's Cathedral."),
            "київ": ("**Розташування:** Столиця України, на річці Дніпро.\n"
                     "**Історія:** Одне з найдавніших міст Східної Європи, засноване у **V столітті**. Був центром Київської Русі. "
                     "Головні пам'ятки: Києво-Печерська лавра та Софійський собор. Місто-герой."),
            "odessa": ("**Location:** Southern Ukraine, a major port on the Black Sea.\n"
                       "**History:** Officially founded in **1794** by Empress Catherine the Great, though a small settlement existed earlier. "
                       "It quickly became a major trade hub in the Russian Empire. Known for the Potemkin Stairs and its unique cultural blend."),
        }
        
        # --- База знань: Країни (розширена) ---
        KNOWN_COUNTRIES = {
            "україна": ("**Столиця:** Київ\n**Населення:** ~41 млн (до 2022)\n**Офіційна мова:** Українська\n**Особливості:** Друга за площею країна в Європі, відома своєю багатою культурою та родючими чорноземами."),
            "ukraine": ("**Capital:** Kyiv\n**Population:** ~41 million (pre-2022)\n**Official Language:** Ukrainian\n**Features:** Second largest country by area in Europe, known for its rich culture and fertile black soil."),
            "france": ("**Столиця:** Париж\n**Населення:** ~68 мільйонів\n**Офіційна мова:** Французька\n**Особливості:** Світовий центр мистецтва, моди, гастрономії. Тут розташовані Ейфелева вежа та Лувр."),
            "японія": ("**Столиця:** Токіо\n**Населення:** ~125 мільйонів\n**Офіційна мова:** Японська\n**Особливості:** Країна висхідного сонця. Відома поєднанням стародавніх традицій та передових технологій."),
            "usa": ("**Столиця:** Вашингтон, округ Колумбія\n**Населення:** ~335 мільйонів\n**Офіційна мова:** Англійська (де-факто)\n**Особливості:** Третя за площею та населенням країна світу. Відома своєю федеративною системою та Нью-Йорком."),
        }
        
        # 1. Спроба відповісти на запит про КРАЇНУ
        if lower_query in KNOWN_COUNTRIES:
             log_message(f"AI INFO: Знайдено країну: {query}")
             return KNOWN_COUNTRIES[lower_query]
        
        # 2. Спроба відповісти на запит про МІСТО
        if lower_query in KNOWN_CITIES:
             log_message(f"AI INFO: Знайдено місто: {query}")
             return f"<p style='color:#f1c40f; font-weight:bold;'>🏢 Інформація про місто {query.title()}:</p><br>" + KNOWN_CITIES[lower_query]
        
        # 3. Універсальна заглушка
        log_message(f"AI WARNING: Запит '{query}' не знайдено у базі знань, повертаю заглушку.")
        return (f"<p style='color:#e74c3c; font-weight:bold;'>Попередження:</p>Я не знайшов детальної інформації про **{query.title()}** у своїй базі. "
                f"Будь ласка, введіть інше місто або країну, або спробуйте уточнити свій запит.")

class AICountryInfoDialog(QWidget):
    """Окреме діалогове вікно для AI-асистента з чатом."""
    def __init__(self):
        super().__init__()
        self.setWindowTitle("AI Асистент: Інформація про Країни та Міста 🤖")
        self.resize(650, 550)
        self.worker = None
        self._setup_ui()
        self._setup_style()
    ...
    def _setup_ui(self):
        ...
        header = QLabel("🤖 AI-Консультант: Запитайте про Країни та Міста")
        ...
        self.chat_history.setHtml(
            ...
            "Привіт! Я ваш AI-асистент. Введіть назву країни (наприклад, **Франція**) або відомого міста (наприклад, **Вінниця**), щоб отримати основну інформацію."
        )
...
class MapWeatherApp(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.ai_assistant_btn = QPushButton("AI Асистент (Країни/Міста) 🤖")
    ...
    def _setup_style(self):
        self.setStyleSheet(f"""
            ...
            QLabel#header_label {{ 
                font-size:40px; /* Збільшено розмір шрифту */
                font-weight:bold; 
                color:white; 
                margin-bottom:10px;
                padding: 10px;
    ...
    def on_ai_assistant(self):
        ...
        if self.ai_assistant_dialog is None:
            self.ai_assistant_dialog = AICountryInfoDialog()
